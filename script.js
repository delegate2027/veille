...
function removeEmojis(str) {
  return str
    .replace(/[\u{1F000}-\u{1FFFF}]|[\u{2600}-\u{27BF}]\uFE0F?|[\u{FE00}-\u{FE0F}]|\p{Emoji_Presentation}|\p{Extended_Pictographic}|\u{200D}|\u{20E3}/gu, "")
    .replace(/\s{2,}/g, " ")
    .trim();
}
...
async function loadRSS() {
  ...
  try {
    ...
    items.forEach((item) => {
      const title = removeEmojis(item.querySelector("title")?.textContent || "");
      const link = item.querySelector("link")?.textContent || "#";
      const author = item.querySelector("author")?.textContent || "Auteur";
      const description = removeEmojis(item.querySelector("description")?.textContent || "");
